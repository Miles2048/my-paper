...
对于本文提出的 PA-DQN 方法而言，策略学习是在环境交互中不断采样、持续更新的序列决策学习过程。本文采用“复杂城市环境仿真构建 + 强化学习预训练 + 收敛性分析”的技术路线，在可控仿真环境中完成 PA-DQN 智能体的初始策略学习。

\subsection{仿真预训练环境构建}
\label{subsec:3_5_1}

仿真预训练环境以 3.2 节构建的三维场景为基础，将目标终端运动、传播环境变化及多源观测生成统一纳入同一交互平台。在训练回合中，系统随机组合轨迹模式与传播状态切换机制，使智能体能接触到更丰富的状态转移分布，从而减弱对单一场景的依赖。

\subsection{神经网络结构与训练参数配置}
\label{subsec:3_5_2}

本文采用三层隐藏层的全连接网络作为逼近器。输入层维数为 6，包含物理感知状态分量及历史权重；隐藏层节点数设为 128/128/64，均采用 ReLU 激活。在训练参数方面，学习率设为 \(1\times 10^{-3}\)，折扣因子 \(\gamma=0.95\)，经验回放池容量为 50000。

表~\ref{tab:pa_dqn_config} 给出了本文 PA-DQN 神经网络结构与主要训练参数配置。

\begin{table}[htbp]
    \centering
    \caption{PA-DQN 神经网络结构与训练参数配置}
    \label{tab:pa_dqn_config}
    \begin{tabular}{lll}
        \toprule
        \textbf{类别} & \textbf{参数} & \textbf{配置} \\
        \midrule
        网络结构 & 输入维数 & 6 \\
         & 隐藏层层数 & 3 \\
         & 隐藏层节点数 & 128 / 128 / 64 \\
        ...
        优化设置 & 优化器 & Adam \\
         ...
         & 损失函数 & 均方误差损失 \\
        \midrule
        强化学习参数 & 折扣因子 \(\gamma\) & 0.95 \\
         & 初始探索率 \(\varepsilon_0\) & 1.0 \\
...
在训练策略上，本文采用“先仿真预训练、后测试评估”的两阶段思路。训练初期通过较大的探索概率 \(\varepsilon\) 鼓励智能体充分尝试动作；随后利用目标网络周期同步机制缓解训练不稳定性，使智能体在正式测试前形成稳定的初始决策能力。

\subsection{收敛性分析指标与过程讨论}
\label{subsec:3_5_4}

本文采用平均回合奖励、平均定位误差和 Q 网络训练损失评估收敛过程。收敛过程经历探索主导、策略形成和稳定收敛三个阶段。实验表明，当训练进入后期后，平均定位误差收敛至较低水平，训练损失保持稳定。
