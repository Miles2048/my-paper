...
除环境和观测特征外，本文还将上一时刻的权重向量 \(\boldsymbol{\omega}_{t-1}\) 作为状态的一部分。引入 \(\boldsymbol{\omega}_{t-1}\) 后，智能体在进行当前决策时能够保留必要的历史上下文，这也为后续增量式动作设计提供了基础。从方法层看，PA-DQN 的状态空间重构使权重分配不再只是对已发生误差的被动修正，而更接近于依据当前环境特征进行前置调节。
...
与离散动作空间相比，增量式连续动作空间使控制量天然具有连续性，更适合单目标轨迹跟踪场景下的时序决策需求。为了进一步限制权重突变幅度，本文对连续动作增量的取值范围设置边界约束，即 \(\Delta \omega_t^{x}\in[-\delta,\delta]\)。这种“缓变更新、持续纠偏”的特性，显著抑制了由策略突变带来的轨迹锯齿现象。

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.84\textwidth]{figures/ch3_continuous_action_space.pdf}
    \vspace{4.5cm}
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.84\textwidth]{figures/ch3_reward_design_principle.pdf}
    \vspace{4.5cm}
    \caption{PA-DQN 奖励函数中精度项与平滑项协同作用机理示意图}
    \label{fig:ch3_reward_design_principle}
\end{figure}
...
为了提升训练过程的稳定性，本文在 PA-DQN 中引入经验回放机制和目标网络机制。经验回放用于打破样本间的时序相关性，而目标网络则减弱了目标值随训练同步波动带来的震荡问题。图~\ref{fig:pa_dqn_network_structure} 给出了 PA-DQN 智能体网络结构与训练流程示意图。
...
为便于后续补充实验图像，图~\ref{fig:ch3_training_convergence} 预留为训练阶段收敛曲线图，建议展示奖励值、训练损失或平均误差随训练轮数变化的趋势；图~\ref{fig:ch3_simulation_scenarios} 预留为四类典型轨迹与场景设置示意图。

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.78\textwidth]{figures/ch3_training_convergence.pdf}
    \vspace{4.8cm}
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.86\textwidth]{figures/ch3_simulation_scenarios.pdf}
    \vspace{5.0cm}
    \caption{复杂城市环境下典型测试轨迹与场景设置示意图}
    \label{fig:ch3_simulation_scenarios}
\end{figure}
...
为了进一步分析不同算法在连续定位任务中的轨迹表现，本文选取矩形路径、对角线路径、反对角线路径以及随机游走路径四类典型测试轨迹进行可视化比较，结果如图~\ref{fig:ch3_typical_trajectories} 所示。为便于后续插入论文中的原始结果图，本图建议采用四幅子图形式，分别对应四类典型轨迹。
...
相比之下，PA-DQN 在四类轨迹上的输出整体更接近真实轨迹。在矩形路径中，转角位置的偏移较小；在对角线和反对角线路径中，穿越 LOS/NLOS 切换区段时轨迹保持较好的连续性；在随机游走路径中，其局部轨迹抖动相对较弱。该结果说明，物理感知状态空间与增量式动作更新机制能够在连续定位场景中有效抑制由观测突变和权重震荡引起的轨迹不连续问题。

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.88\textwidth]{figures/ch3_typical_trajectories.pdf}
    \vspace{5.2cm}
    \caption{不同算法在典型测试轨迹下的连续定位结果对比}
    \label{fig:ch3_typical_trajectories}
\end{figure}

此外，为进一步突出环境切换区段的局部行为，图~\ref{fig:ch3_local_zoom} 预留为典型轨迹局部放大图，可重点展示 LOS/NLOS 边界附近不同算法的局部偏移与轨迹平滑性差异。
...
这一结果与前文方法设计是一致的。由于 PA-DQN 在决策过程中不仅利用后验误差，还引入了 RMS 时延扩展、RSSI 方差和几何一致性等状态特征，因此在信道质量下降时，智能体能够更早地感知观测退化趋势，并通过权重调整减弱不可靠观测对融合结果的影响。由此，其在低信噪比条件下仍能保持较稳定的性能输出。

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.78\textwidth]{figures/ch3_snr_rmse_curve.pdf}
    \vspace{4.8cm}
    \caption{不同算法在多种信噪比条件下的 RMSE 对比曲线}
    \label{fig:ch3_snr_rmse_curve}
\end{figure}

为补充说明不同算法在尾部误差上的变化，图~\ref{fig:ch3_snr_p95_curve} 预留为不同信噪比条件下的 P95 对比曲线图，用于展示各算法在恶劣信道条件下的尾部鲁棒性差异。
...
为分析 PA-DQN 各关键模块的独立作用，本文进一步开展消融实验。具体而言，分别移除物理感知状态空间、增量式动作空间以及稳定性奖励项，并比较其性能变化，结果如表~\ref{tab:ch3_ablation} 所示。

\begin{table}[htbp]
    \centering
    \caption{PA-DQN 各关键模块的消融实验结果}
    \label{tab:ch3_ablation}
    \begin{tabular}{lcc}
        \toprule
        \textbf{模型变体} & \textbf{RMSE (m)} & \textbf{稳定性指标} \\
        \midrule
...
为便于后续补充更加直观的图形结果，图~\ref{fig:ch3_ablation_bar} 预留为消融实验柱状图，可分别展示不同模块移除后 RMSE 和稳定性指标的变化情况。
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.86\textwidth]{figures/ch3_typical_trajectories.pdf}
    \vspace{5.0cm}
    \caption{不同算法在典型测试轨迹下的连续定位结果对比}
    \label{fig:ch3_typical_trajectories}
\end{figure}
...
这一现象与前文方法设计是一致的。由于 PA-DQN 在决策过程中不仅利用后验误差，还引入了 RMS 时延扩展、RSSI 方差和几何一致性等状态特征，因此在信道质量下降时，智能体能够更早地感知观测退化趋势，并通过权重调整减弱不可靠观测对融合结果的影响。由此，其在低信噪比条件下仍能保持较为稳定的性能输出。
...
为分析 PA-DQN 中各关键模块的独立作用，本文进一步开展消融实验。具体而言，分别移除物理感知状态空间、增量式动作空间以及稳定性奖励项，并比较其性能变化，结果如表~\ref{tab:ch3_ablation} 所示。
...
本节通过总体性能统计、典型轨迹可视化、不同信噪比条件下的误差曲线以及消融实验，对 PA-DQN 的实验表现进行了分析。结果表明，PA-DQN 在总体精度、尾部误差控制、低信噪比条件下的稳定性以及连续轨迹平滑性方面均优于对比方法。实验现象与前文方法设计保持一致，说明将物理感知状态空间、增量式动作更新机制和联合奖励函数引入强化学习融合决策后，能够更好地适应复杂城市环境中的观测退化与状态切换过程。