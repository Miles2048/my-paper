...
现有公开研究表明，Starlink 在 Ku 波段下行链路采用正交频分复用（OFDM）体制，其信号在频域上表现为边界清晰、带内功率近似平坦的多子载波结构\cite{Humphreys2024Starlink,Humphreys2023StarlinkSignal,Kozhaya2025StarlinkPNT}。结合前文给出的多径传播模型，可将第 $m$ 帧中第 $i$ 个 OFDM 符号在接收端的复基带信号统一表示为
\begin{equation}
r_{m,i}(t)
=
\sum_{l=1}^{L}
...
基于上述考虑，本节介绍与后续方法直接相关的两类理论基础：面向动态定位决策的马尔可夫决策过程与 DQN 强化学习基础，以及面向残缺观测鲁棒定位的知识蒸馏基础。其中，前两部分主要服务于第三章中的单目标自适应融合定位建模，后一部分主要服务于第四章中的多目标协同鲁棒定位建模。
...
综上，马尔可夫决策过程为复杂环境下的动态定位问题提供了统一描述方式。通过将定位状态感知、策略选择与奖励反馈纳入同一闭环框架，MDP 为后续第三章中基于强化学习的多源异构融合定位方法奠定了理论基础。在本文中，MDP 并非抽象的通用决策模型，而是对单目标连续定位中“状态感知---权重调整---位置更新”闭环过程的形式化描述。
...
综上，DQN 在保留强化学习长期决策优势的基础上，利用深度神经网络实现了对高维定位状态空间中价值函数的有效逼近。对于本文第三章所研究的动态融合定位问题，DQN 提供了将复杂环境感知状态直接映射为融合决策动作的可行工具。具体到本文场景中，DQN 所承担的核心角色并非一般动作选择器，而是面向复杂环境下多源观测融合权重动态调整的定位决策器。
