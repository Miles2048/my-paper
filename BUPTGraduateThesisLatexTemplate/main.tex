\documentclass[master,eng,fulltime,electronic,cenhdr,normaltoc,macos]{buptthesis} % 专硕电子版
%\documentclass[master,eng,fulltime,electronic,cenhdr,normaltoc,blind]{buptthesis} % 专硕电子版
%\documentclass[master,phil,fulltime,electronic,cenhdr,normaltoc,blind]{buptthesis} % 学硕电子版
%\documentclass[master,eng,fulltime,electronic,cenhdr,normaltoc]{buptthesis} % 专硕电子版
%\documentclass[master,eng,fulltime,print,cenhdr,normaltoc]{buptthesis} % 专硕印刷版
%\documentclass[phd,phil,fulltime,electronic,cenhdr,normalto]{buptthesis} % 学博电子版
%\documentclass[phd,phil,fulltime,print,cenhdr,normaltoc]{buptthesis} % 学博印刷版
%\documentclass[phd,eng,fulltime,electronic,cenhdr,normaltoc]{buptthesis} % 专博电子版
%\documentclass[phd,eng,fulltime,print,cenhdr,normaltoc]{buptthesis} % 专博印刷版

% !UPDATE 20231210: add to remove `normaltoc` to remove table content in the toc

% 电子版和印刷版的区别
% 1. 电子版是没有跳偶数页，所有内容都是直接连起来的
% 2. 印刷版部分内容跳偶数页开头的，生成的PDF直接双页打印即可

% 盲审版在选项后加入`blind`，比如学硕电子版的盲审版指令如下：
% \documentclass[master,phil,fulltime,electronic,cenhdr,normaltoc,blind]{buptthesis} % 学硕电子版

\makeatletter
% 检查是否为盲审版本
\newif\ifWithOptionBlind 
\@ifclasswith{buptthesis}{blind}{\WithOptionBlindtrue}{\WithOptionBlindfalse}

% 双面打印时，前置单页材料背面留空。
\newcommand{\BUPTblankprepage}{%
  \thispagestyle{empty}%
  \null
  \newpage
}

\renewcommand{\makeprepages}{%
  \makechinesetitle
  \BUPTblankprepage
  \makeenglishtitle
  \BUPTblankprepage
  \makecommitteelist
  \BUPTblankprepage
  \ifBUPT@option@blind \else
    \makestatement
    \BUPTblankprepage
  \fi
}
\makeatother

% 导入宏包
\usepackage{graphicx}
\graphicspath{{figures/}}
\usepackage{bbding}
\usepackage{booktabs}
\usepackage{bicaption}
\usepackage{float}
\usepackage{gbt7714}
\usepackage{fontspec}

% 定义使用的函数/宏
\DeclareMathOperator{\Tr}{Tr}

% 正文当中的其他定义
\setmainfont{Times New Roman} % 英文字体使用Times New Roman
\setmathfont{latinmodern-math.otf} % 数学公式字体使用 TeX Live 自带字体，避免依赖系统字体
\BUPTthesiscntitlepage{%
	confidential = {公开}, % 目前只支持公开
	title = {基于无人机载平台的\\卫星互联网终端高精度定位方法},  % 本章标题
	studentid = {2023140074}, % 学号
	name = {石志鹏}, % 姓名
	major = {通信工程}, % 专业名称
	supervisor = {黄赛}, % 导师姓名
	institute = {信息与通信工程学院} % 单行学院的写法
}

\BUPTdateOptions{%
    pageYear = {2026}, % 封面上面的年
    pageMonth = {3}, % 封面上面的月
    pageDay = {11}, % 封面上面的天
    dissertationYear = {2026}, % 答辩时间的年
    dissertationMonth = {5}, % 答辩时间的月
    dissertationDay = {25} % 答辩时间的天
}

\BUPTdissertationcommitteepage{%
    committeeChairName = {张晗},
    committeeChairTitle = {岗位特聘教授（人才引进）},
    committeeChairOrgan = {北京邮电大学},
    committeeMemberFirstName = {昌硕},
    committeeMemberFirstTitle = {讲师（高校）},
    committeeMemberFirstOrgan = {北京邮电大学},
    committeeMemberSecondName = {姜蔚蔚},
    committeeMemberSecondTitle = {副教授},
    committeeMemberSecondOrgan = {北京邮电大学},
    committeeMemberThirdName = {李蓉},
    committeeMemberThirdTitle = {高级工程师},
    committeeMemberThirdOrgan = {国家无线电监测中心},
    committeeSecretaryName = {骆一阳},
    committeeSecretaryTitle = {},
    committeeSecretaryOrgan = {北京邮电大学}
}

\BUPTthesisentitlepage{%
	confidentialEN = {Public},  % 目前只支持公开
	titleEN = {High-Accuracy Localization Methods for Satellite Internet \\Terminals Based on UAV-Borne Platforms}, % 本章标题
	nameEN = {Zhipeng Shi}, % 姓名
	majorEN = {Communication Engineering}, % 专业名称
	supervisorEN = {Sai Huang}, % 导师姓名
	instituteEN = {School of Information and Communication Engineering} % 学院名称
}

\begin{document}

\makeprepages

\frontmatter
\include{Chapter/Chapter_cnabstract} % 中文摘要
\include{Chapter/Chapter_enabstract} % 英文摘要
\tableofcontents % 目录
\include{Chapter/Chapter_nomeclature} % 符号表

\mainmatter
\include{Chapter/Chapter_01_Introduction} % 第一章：绪论
\include{Chapter/Chapter_02_Fundamentals} % 第二章：系统模型与定位理论基础
% 原第三章（单目标射频指纹定位）已被精简整合，故移除独立章节
\include{Chapter/Chapter_04_Fusion} % 第三章：基于一致性增强监督权重学习的单目标自适应融合定位方法
\include{Chapter/Chapter_05_Multi_Target} % 第四章：基于空间信号表征的多目标协同鲁棒定位方法
\include{Chapter/Chapter_06_Conclusion} % 第五章：结论与展望

\bibliographystyle{gbt7714-numerical}
{\zihao{5}
\bibliography{Bib/thesis} % 参考文献
}

\backmatter
\include{Chapter/Chapter_abbreviation} % 缩写附录
\thispagestyle{empty}
\mbox{}
\clearpage
\ifWithOptionBlind \else
\include{Chapter/Chapter_acknowledgement} % 致谢
\fi
\include{Chapter/Chapter_publication} % 创新成果
\ifWithOptionBlind \else
\include{Chapter/Chapter_experience} % 作者学习经历
\fi

\end{document}
